...
public:
	...
	static inline void Node(Chess::PositionStack& stack, const Chess::Move& move)
	{
		constexpr Chess::U8 depth = DEPTH - 1;
		Chess::MoveGenerator::EnumerateMoves<AddToMoveList, C, EP, depth, OC>(stack, move);
	}
	static inline void LeafNode(Chess::PositionStack& stack, const Chess::Move& move)
	{
...
int main()
{
	...
	Glue::Callbacks::StartGame = []() {
		...
		Chess::LoadFEN(game.stack, "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
	};

	Glue::Callbacks::MakeMove = [](Glue::Callbacks::Move move) {
		moveList.clear();
		Chess::MoveGenerator::EnumerateMoves<AddToMoveList, 1>(game.stack);
		const auto real_move = Chess::FindMove(moveList, move.start_square, move.end_square);
		Chess::MakeMove(game.stack, real_move);
	};

	Glue::Callbacks::Think = [](Glue::Callbacks::ThinkArgs args) -> Glue::Callbacks::ThinkResult {
		Chess::Move bookMove{};
		if(game.stack.size() < Chess::Book::NUM_MOVES_IN_BOOK)
		{
			...
			if(Chess::Move::Valid(bookMove))
			{
				...
				return Glue::Callbacks::ThinkResult{
					.best_move = Glue::Callbacks::Move{
						bookMove.startSquare, bookMove.endSquare,
					},
					.eval = NAN,
				};
			}
		...
		t.Start();
		Chess::Engine::Evaluation eval = Chess::Engine::Search<6, 0>(game.stack, Chess::Engine::NegativeInfinity, Chess::Engine::Infinity);
		t.End();
		...
		return Glue::Callbacks::ThinkResult{
			.best_move = Glue::Callbacks::Move{
				bestMove.startSquare, bestMove.endSquare,
			},
			.eval = (double)eval,
		};
	};
	Glue::Callbacks::ValidMoves = [](int start_square) -> std::vector<int> {
		...
		for(const Chess::Move& move : moveList) {
			if(move.startSquare == start_square) {
				...
				std::cout << "move.endSquare = " << (Chess::U64)move.endSquare << std::endl;
			}
		};
		return moves;
	};
	Glue::Callbacks::QueryBoard = []() -> std::array<int, 64> {
		...
		for(const Chess::Piece piece : game.stack.top().pieces) {
			pieces[i++] = (piece.color == Chess::Piece::Color::Black ? 16 : 8) + piece.type + 1;
		}
		return pieces;
	};
}
