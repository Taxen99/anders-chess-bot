...
public:
	...
	static inline void Node(PositionStack& stack, const Move& move)
	{
		...
		MoveGenerator::EnumerateMoves<IncrementMoveTotal, C, EP, depth, OC>(stack, move);
	}
...
template<U8 depth>
static U64 _Perft(PositionStack& stack)
{
...
template<U8 depth>
U64 Perft(const std::string& FEN)
{
	PositionStack stack{};
	LoadFEN(stack, FEN);
