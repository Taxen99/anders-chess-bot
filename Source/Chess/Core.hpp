...
inline U64 CharsToBB(char f, char r)
{
   return CharToFile(f) & CharToRank(r);
}
...
public:
   ...
   inline void End()
   {
      ...
      std::cout << m_label << " : " << std::chrono::duration_cast<std::chrono::milliseconds>(m_end - m_begin).count() << "[ms]" << std::endl;
   }
private:
   std::string m_label;
