...
void LoadFEN(PositionStack& stack, const std::string& FEN)
{
	...
	position.castlingRights[Piece::Black] = { .kingSide = false, .queenSide = false };
	position.castlingRights[Piece::White] = { .kingSide = false, .queenSide = false };
	if(castles == "-")
	{
	...
	else
	{
		...
		const char epRank = epCaptureSquare.at(1) + (position.turn == Piece::White ? -1 : 1) - 49;
...
static inline U64 GetNonSlidingAttackers(const Position& position, U8 index)
{
	...
	return (KnightMoves[index] & enemyKnights) | (PawnAttacks[position.turn][index] & enemyPawns);
}

void CalculateCheckAndPinMask(const Position& position, U64& checkMask, U64& pinHVMask, U64& pinDIMask)
{
	checkMask = 0, pinHVMask = 0, pinDIMask = 0;
	const U64 kingBB = position.pieceBB[Piece::King] & position.colorBB[position.turn];
	...
	U64 pieceSouthRay 		= soutOccl((position.pieceBB[Piece::Rook] 	| position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceNorthRay 		= nortOccl((position.pieceBB[Piece::Rook] 	| position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceWestRay 		= westOccl((position.pieceBB[Piece::Rook] 	| position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceSouthWestRay 	= soWeOccl((position.pieceBB[Piece::Bishop] | position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceNorthWestRay 	= noWeOccl((position.pieceBB[Piece::Bishop] | position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceEastRay 		= eastOccl((position.pieceBB[Piece::Rook] 	| position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceNorthEastRay 	= noEaOccl((position.pieceBB[Piece::Bishop] | position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
	U64 pieceSouthEastRay 	= soEaOccl((position.pieceBB[Piece::Bishop] | position.pieceBB[Piece::Queen]) & enemyBB, emptyBB);
...
#define TRACK(x) x;std::cout<<#x<<std::endl
...
void MakeMove(PositionStack& stack, const Move& move)
{
	...
	if(position.occupiedBB & endBB) // This is a capture
	{
		...
		position.zobrist ^= Zobrist::Pieces[capturePiece.type][capturePiece.color][move.endSquare]; // ZOBRIST
	}
	...
	if(move.promotion == Piece::None)
	{
		...
		position.zobrist ^= Zobrist::Pieces[piece.type][piece.color][move.startSquare]; // ZOBRIST
		position.zobrist ^= Zobrist::Pieces[piece.type][piece.color][move.endSquare]; // ZOBRIST
	}
	else
	{
		...
		position.zobrist ^= Zobrist::Pieces[piece.type][piece.color][move.startSquare]; // ZOBRIST
		position.zobrist ^= Zobrist::Pieces[move.promotion][piece.color][move.endSquare]; // ZOBRIST
	...
	if(move.castlingSide == CastlingSide::KingSide)
	{
		...
		const U64 rookSwapBB = castlingRookMask[piece.color].kingSide ^ castlingRookEnds[piece.color].kingSide;
		position.pieceBB[Piece::Rook] ^= rookSwapBB;
		position.colorBB[piece.color] ^= rookSwapBB;
		position.occupiedBB			 ^= rookSwapBB;

		position.zobrist ^= Zobrist::Pieces[Piece::Rook][piece.color][rsi]; // ZOBRIST
		position.zobrist ^= Zobrist::Pieces[Piece::Rook][piece.color][rei]; // ZOBRIST
	}
	else if(move.castlingSide == CastlingSide::QueenSide)
	{
		...
		position.pieces[rsi] = Piece::NoPiece;
		position.pieces[rei] = { .type = Piece::Rook, .color = piece.color };
		const U64 rookSwapBB = castlingRookMask[piece.color].queenSide ^ castlingRookEnds[piece.color].queenSide;
		position.pieceBB[Piece::Rook] ^= rookSwapBB;
		position.colorBB[piece.color] ^= rookSwapBB;
		position.occupiedBB			  ^= rookSwapBB;
	...
	if(move.takeEnPassant)
	{
		...
		position.zobrist ^= Zobrist::Pieces[Piece::Pawn][REVERSE_TURN(position.turn)][epIndex]; // ZOBRIST
	}

	if(piece.type == Piece::King)
	{
		if(position.castlingRights[piece.color].kingSide)
			position.zobrist ^= Zobrist::Castling[0 + piece.color * 2]; // 0 for black & 2 for white ZOBRIST
		if(position.castlingRights[piece.color].queenSide)
			position.zobrist ^= Zobrist::Castling[1 + piece.color * 2]; // 1 for black & 3 for white ZOBRIST
	...
	if(move.doublePush)
	{
		...
		{
			position.zobrist ^= Zobrist::EnPassant[bitScanForward(position.enPassantBB) & 7]; // ZOBRIST
		}
