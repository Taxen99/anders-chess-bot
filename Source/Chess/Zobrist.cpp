...
U64 Zobrist::Hash(const Position &position)
{
	...
	{
		zobrist ^= EnPassant[bitScanForward(position.enPassantBB) & 7]; // (EP index) mod 8 (aka ep file index)
	}
	if(position.castlingRights[Piece::White].kingSide) // BBWW (0 + 2c)k (1 + 2c)q
		zobrist ^= Castling[2];
