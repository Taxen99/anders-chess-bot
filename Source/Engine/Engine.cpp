...
static constexpr Evaluation PieceValue()
{
...
Evaluation Evaluate(Chess::PositionStack& stack)
{
	...
	// U64 fKnights = position.Get<Piece::Knight>() & position.colorBB[friendlyColor];
	// U64 fBishops = position.Get<Piece::Bishop>() & position.colorBB[friendlyColor];
	// U64 fRooks = position.Get<Piece::Rook>() & position.colorBB[friendlyColor];
	// U64 fQueens = position.Get<Piece::Queen>() & position.colorBB[friendlyColor];

	// U64 ePawns = position.Get<Piece::Pawn>() & position.colorBB[enemyColor];
	// U64 eKnights = position.Get<Piece::Knight>() & position.colorBB[enemyColor];
	// U64 eBishops = position.Get<Piece::Bishop>() & position.colorBB[enemyColor];
	// U64 eRooks = position.Get<Piece::Rook>() & position.colorBB[enemyColor];
	// U64 eQueens = position.Get<Piece::Queen>() & position.colorBB[enemyColor];

	// Evaluation eval = 0;
	// if(fPawns) do { eval += PieceValue<Piece::Pawn>(); } while(fPawns &= fPawns - 1);
	// if(fKnights) do { eval += PieceValue<Piece::Knight>(); } while(fKnights &= fKnights - 1);
	// if(fBishops) do { eval += PieceValue<Piece::Bishop>(); } while(fBishops &= fBishops - 1);
	// if(fRooks) do { eval += PieceValue<Piece::Rook>(); } while(fRooks &= fRooks - 1);
	// if(fQueens) do { eval += PieceValue<Piece::Queen>(); } while(fQueens &= fQueens - 1);

	// if(ePawns) do { eval -= PieceValue<Piece::Pawn>(); } while(ePawns &= ePawns - 1);
	// if(eKnights) do { eval -= PieceValue<Piece::Knight>(); } while(eKnights &= eKnights - 1);
	// if(eBishops) do { eval -= PieceValue<Piece::Bishop>(); } while(eBishops &= eBishops - 1);
	// if(eRooks) do { eval -= PieceValue<Piece::Rook>(); } while(eRooks &= eRooks - 1);
	// if(eQueens) do { eval -= PieceValue<Piece::Queen>(); } while(eQueens &= eQueens - 1);
	...
	for (size_t i = 0; i < moveList.size() - 1; i++) {
		for (int j = i + 1; j > 0; j--) {
			...
			if (moveScores[swapIndex] < moveScores[j]) {
				std::swap(moveList[swapIndex], moveList[j]); // TODO : DON'T USE STD::SWAP !!!
...
void OrderMoves(MoveList &moveList, const Position &position)
{
	...
	const U64 enemyPawns = position.Get<Piece::Pawn>() & position.colorBB[enemyTurn];
...
Evaluation QuietSearch(Chess::PositionStack &stack, Evaluation alpha, Evaluation beta)
{
