...
Evaluation Search(Chess::PositionStack& stack, Evaluation alpha, Evaluation beta)
{
	...
	else
	{
		...
		{
			MakeMove(stack, move);
			const Evaluation eval = -Search<depthMinusOne, depthFromRootPlusOne>(stack, -beta, -alpha);
			PopPosition(stack);
