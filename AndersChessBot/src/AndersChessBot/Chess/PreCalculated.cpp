...
void PreCalculateMoveData() {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    DIRays[i] |=
        (Rays[i][Direction::NorthEast] = noEaAttacks(1ULL << i, ~0ULL));
    DIRays[i] |=
        (Rays[i][Direction::SouthEast] = soEaAttacks(1ULL << i, ~0ULL));
    DIRays[i] |=
        (Rays[i][Direction::SouthWest] = soWeAttacks(1ULL << i, ~0ULL));
    DIRays[i] |=
        (Rays[i][Direction::NorthWest] = noWeAttacks(1ULL << i, ~0ULL));
  }
}
