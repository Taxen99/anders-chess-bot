...
private:
  ...
  static void GenerateMovesTemplate(PositionStack &stack) {
...

    {
      ...
      if constexpr (onlyCaptures == false) {
        if (position.castlingRights[friendlyColor].kingSide) {
          if (!(castlingCheckMask[friendlyColor].kingSide & attackBB) &&
              !(castlingObstructMask[friendlyColor].kingSide &
                position.occupiedBB))
            HandleMove<CALLBACK, C, false, depth, OC>(
                stack, {kingIndex, castlingEndSquares[friendlyColor].kingSide,
                        Piece::None, CastlingSide::KingSide});
        }
        if (position.castlingRights[friendlyColor].queenSide) {
          if (!(castlingCheckMask[friendlyColor].queenSide & attackBB) &&
              !(castlingObstructMask[friendlyColor].queenSide &
                position.occupiedBB))
            HandleMove<CALLBACK, C, false, depth, OC>(
                stack, {kingIndex, castlingEndSquares[friendlyColor].queenSide,
                        Piece::None, CastlingSide::QueenSide});
        }
      }
    }
    ...
    U64 knights_ = position.Get<friendlyColor, Piece::Knight>() & (~pinHVMask) &
                   (~pinDIMask);
    U64 bishops_ =
        (position.Get<Piece::Bishop>() | position.Get<Piece::Queen>()) &
        friendlyBB & ~pinHVMask;
    U64 rooks_ = (position.Get<Piece::Rook>() | position.Get<Piece::Queen>()) &
                 friendlyBB & ~pinDIMask;

    constexpr U64 promotionRank =
        friendlyColor == Piece::White ? eigthRank : firstRank;
    ...
    if (pawns_)
      do {
        ...
        U64 doublePawnPushes =
            PushPawns1(pushMoves, empty, friendlyColor) & doublePushRank;
        ...
        // if any of the moves are a promotion, then all of them must be
        // promotions also we can ignore en passant & double pushes here since
        // you can't en passant or double push & promote at the same time
        ...
        if (moves & promotionRank) {
          if (moves)
            do {
              U8 idx = bitScanForward(moves);
              HandleMove<CALLBACK, C, false, depth, OC>(
                  stack, {pawnIndex, idx, Piece::Knight});
              HandleMove<CALLBACK, C, false, depth, OC>(
                  stack, {pawnIndex, idx, Piece::Bishop});
              HandleMove<CALLBACK, C, false, depth, OC>(
                  stack, {pawnIndex, idx, Piece::Rook});
              HandleMove<CALLBACK, C, false, depth, OC>(
                  stack, {pawnIndex, idx, Piece::Queen});
            } while (moves &= moves - 1);
        } else {
          if (moves)
            do {
              U8 idx = bitScanForward(moves);
              HandleMove<CALLBACK, C, false, depth, OC>(stack,
                                                        {pawnIndex, idx});
            } while (moves &= moves - 1);

          if constexpr (onlyCaptures == false) {
            if (doublePawnPushes) {
              HandleMove<CALLBACK, C, true, depth, OC>(
                  stack, {pawnIndex, bitScanForward(doublePawnPushes),
                          Piece::None, CastlingSide::None, true});
            }
          }
        }
        ...
        if constexpr (hasEP) {
          U64 enPassantTake =
              PushPawns1(position.enPassantBB, empty, friendlyColor);
          U64 enPassantMove =
              PawnAttacks[friendlyColor][pawnIndex] & enPassantTake;
          ...
          if (enPassantMove && (position.enPassantBB & trueCheckMask)) {
            ...
            const U64 enemyRooksAndQueens =
                (position.Get<Piece::Rook>() | position.Get<Piece::Queen>()) &
            ...
            if ((epClearRank & kingBB) && enemyRQOnEPRank) {
              ...
              const U64 emptyNoEnPassantPawns =
                  empty | (pawn | position.enPassantBB);
            ...
            HandleMove<CALLBACK, C, false, depth, OC>(
                stack, {pawnIndex, bitScanForward(enPassantMove), Piece::None,
                        CastlingSide::None, false, true});
          }
        }
    ...
    if (bishops_)
      do {
        ...
        U64 moves =
            Bmagic(bishopIndex, position.occupiedBB) & valid & trueCheckMask;
    ...
    if (rooks_)
      do {
        ...
        U64 moves =
            Rmagic(rookIndex, position.occupiedBB) & valid & trueCheckMask;
    ...
    ComputedPositionData.check =
        check; // FIXME: Is this a bug in case of early return?
  }

private:
  template <Piece::Color C> static U64 GetAttackBB(const Position &position) {
    ...
    U64 enemyBishopBB =
        ...
        enemyBB;
    U64 enemyRookBB =
        (position.Get<Piece::Rook>() | position.Get<Piece::Queen>()) & enemyBB;
    const U8 enemyKingIndex =
        bitScanForward(position.Get<Piece::King>() & enemyBB);
