...
void OpeningBook::LoadGamaDatabase(const char *data, size_t size) {
  m_Initialized = true;
  const char *ptr = data;
...
Move OpeningBook::LookUpPosition(const Position &position) {
  ...
  std::vector<Move> out;
  size_t nelems = 1;
  std::sample(possibleMoves.begin(), possibleMoves.end(),
...
Move OpeningBook::ParseSanatizedSAN(const char *SAN, PositionStack &stack) {
  ...
  if (SAN[0] == 'O') {
    const U8 kingIndex = bitScanForward(position.Get<Piece::King>() &
                                        position.colorBB[friendlyColor]);
    if (len == 3)
      return {kingIndex, castlingEndSquares[friendlyColor].kingSide,
              Piece::None, CastlingSide::KingSide};
    if (len == 5)
      return {kingIndex, castlingEndSquares[friendlyColor].queenSide,
  ...
  if (SAN[len - 2] == '=') {
    ...
    if (len == 4) {
      ...
      for (size_t i = 0; i < moveList.size(); i++) {
        ...
        if ((position.Get<Piece::Pawn>() & (1ULL << move.startSquare)) &&
            endBB == (1ULL << move.endSquare) &&
  ...
  if (isupper(SAN[0])) {
    if (len == 3) {
      ...
      for (size_t i = 0; i < moveList.size(); i++) {
        const Move &move = moveList[i];
        if ((position.pieceBB[pieceType] & (1ULL << move.startSquare)) &&
            endBB == (1ULL << move.endSquare))
          return move;
      }
      assert(false);
      ...
      for (size_t i = 0; i < moveList.size(); i++) {
        const Move &move = moveList[i];
        if ((position.pieceBB[pieceType] & (1ULL << move.startSquare) & mask) &&
            endBB == (1ULL << move.endSquare))
          return move;
      }
      assert(false);
